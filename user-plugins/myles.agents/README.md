...
- **Subscription switch** — one chip per enabled agent (`h`/`l`, `[`/`]`, previous/next buttons, or click).
...
- **Daily trend** — compares today's tokens with the average of up to seven
  previous days present in the collector record. The label says how many
  recorded days are included; missing dates are not treated as zero usage.
- **Copy summary** — copies the selected agent's scope, tracked tokens, today's
  tokens, provider message count, and sessions as plain text.
- **Recent tokens by day** — one row per available recent day: day, bar, tokens,
  with today bolded. Hover today for its prompt and session count.
- **Recorded tokens by model** — tokens per model with the bar behind each row scaled
...
| `codex` | The Codex app-server RPC | native Codex CLI sessions touched in 30 days plus Pi/OMP sessions routed to Codex |
| `opencode` | — | all assistant messages with usage in the local OpenCode database, across provider IDs |
...
- Panel: overview `↑`/`↓` selects an agent, Enter opens it, and `1`–`4` sorts by
  tracked tokens, today, provider messages/model turns, or sessions. In detail,
  `h`/`l`, `[`/`]`, `←`/`→`, or the previous/next buttons switch agent; `j`/`k`
  scroll, `r` refreshes, Tab moves to the neighboring bar panel, and Esc returns
  to the overview or closes it.
...
Usage totals describe the records available to each collector. Codex reads
native session files touched in the last 30 days plus retained Pi/OMP sessions
routed to Codex. Fireworks requests the last 30 days from its billing API.
OpenCode reads all assistant messages in its local database across providers;
that local processing volume includes cached and repeated context and is not
account billing. OpenCode normalizes `tokens.input` to uncached input and
stores cache reads/writes separately; the collector adds those disjoint fields
once. Codex also subtracts cached input from its input bucket before adding the
cache bucket. Cached context can recur on later turns, so these totals measure
cumulative processing volume, not unique prompt size or account charges. Claude
reads available local transcripts and stats files. Sources and history windows
can overlap, so the overview shows each provider's
relative recorded volume and does not add them into a purported unique fleet
sum. Counts are provider messages or model turns, not user prompts. Hover over
compact overview values for exact counts and source details.
