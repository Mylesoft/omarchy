...
Item {
  ...
  // Omarchy's known coding agents (launchable CLIs) plus Fireworks (usage /
  // billing collector only). Display names match omarchy-default-agent.
  ...
  // Agents whose binaries are on PATH (or Fireworks with a local record).
  property var installedAgentIds: []
  property int installedRevision: 0
  ...
  Process {
    id: installedScan
    running: false
    // login shell so mise / user PATH sees agent CLIs installed outside /usr
    command: ["bash", "-lc",
      "for a in pi omp opencode ori claude codex crush grok openclaw agy hermes copilot muse cursor-agent; do " +
      "command -v \"$a\" >/dev/null 2>&1 && printf '%s\\n' \"$a\"; done; " +
      "f=\"${XDG_STATE_HOME:-$HOME/.local/state}/omarchy/agents/usage/fireworks.json\"; " +
      "if [ -f \"$f\" ] || [ -f \"$HOME/.fireworks/auth.ini\" ]; then printf 'fireworks\\n'; fi"]
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        ids.sort()
        if (JSON.stringify(ids) !== JSON.stringify(root.installedAgentIds)) {
          root.installedAgentIds = ids
          root.installedRevision++
        }
      }
    }
  }
  ...
  function rescanAgents() {
    ...
    if (!installedScan.running) installedScan.running = true
  }

  function agentDisplayName(id) {
    var key = String(id || "")
    if (root.agentCatalog[key]) return root.agentCatalog[key]
    if (!key) return ""
    return key.charAt(0).toUpperCase() + key.slice(1)
  }

  function emptyProviderRecord(id) {
    return {
      id: id,
      ...
      providerIDs: [],
      recentDays: [],
      usageStatusText: "Installed · no usage data yet",
      authHelpText: ""
    }
  }
  ...
  // settings (default: on). Installed CLIs always appear — even with 0 tokens —
  // so the fleet list matches what is available on this machine.
  property var enabledProviders: {
    var rev = dataRevision
    var syncRev = syncRevision
    var installedRev = installedRevision
    var result = []
    var localIds = {}
    for (var i = 0; i < agents.length; i++) {
      var record = agents[i] ? agents[i].record : null
      ...
      result.push(displayProvider(record))
    }
    // Every CLI present on PATH (plus Fireworks when it has a record) shows up
    // even before a collector writes usage JSON.
    for (var ii = 0; ii < installedAgentIds.length; ii++) {
      var installedId = String(installedAgentIds[ii])
      if (localIds[installedId] || !providerEnabled(installedId)) continue
      localIds[installedId] = true
      result.push(displayProvider(emptyProviderRecord(installedId)))
    }
    // Settings can enable an agent before install / first scan — show a
    // placeholder so the fleet list matches what the user turned on.
    var configured = settings && settings.providers ? settings.providers : {}
    for (var configuredId in configured) {
      if (localIds[configuredId] || !providerEnabled(configuredId)) continue
      if (configured[configuredId] && configured[configuredId].enabled === false) continue
      localIds[configuredId] = true
      result.push(displayProvider(emptyProviderRecord(configuredId)))
    }
    ...
    for (var syncedId in syncedProviders) {
      ...
      var syncedDisplay = displayProvider({
        id: syncedId,
        name: stats.providerName || agentDisplayName(syncedId)
      })
  ...
  function displayProvider(record) {
    ...
    return {
      ...
      providerName: String(record.name || agentDisplayName(record.id) || record.id),
      ready: record.ready === true || synced,
      launchable: String(record.id) !== "fireworks",
      ...
      todayHourActivity: Array.isArray(record.todayHourActivity) ? record.todayHourActivity : [],
      ...
      providerIDs: synced ? (stats.providerIDs || []) : (record.providerIDs || []),
  ...
  function aggregateSnapshots(snapshots) {
      ...
      for (var providerId in snapshotProviders) {
        ...
        for (var r = 0; r < recent.length; r++) {
          ...
          if (acc.recentByDay[date] !== undefined) {
            var dayTokens = day.tokens !== undefined && day.tokens !== null ? day.tokens : day.messageCount
            acc.recentByDay[date] = combineNumber(additive, acc.recentByDay[date], dayTokens)
            var dayMessages = day.providerMessages !== undefined && day.providerMessages !== null
              ? day.providerMessages : day.prompts
            if (dayMessages !== undefined && dayMessages !== null) {
              if (!acc.recentMessagesByDay) acc.recentMessagesByDay = ({})
              acc.recentMessagesByDay[date] = combineNumber(additive, acc.recentMessagesByDay[date], dayMessages)
            }
          }
        }

        var recordedProviderIds = Array.isArray(stats.providerIDs) ? stats.providerIDs : []
        for (var pi = 0; pi < recordedProviderIds.length; pi++)
          acc.providerIDs[String(recordedProviderIds[pi])] = true
    ...
    for (var id in providers) {
      ...
      for (var di = 0; di < dates.length; di++) {
        var date = dates[di]
        var dayTokens = acc.recentByDay[date] || 0
        var dayMessages = acc.recentMessagesByDay[date]
        recentDays.push({ date: date, messageCount: dayTokens, tokens: dayTokens,
          providerMessages: dayMessages === undefined ? null : dayMessages })
      }
      ...
      outProviders[id] = {
        ...
        providerIDs: Object.keys(acc.providerIDs || {}).sort(),
  ...
  function providerSnapshot(record) {
    return {
      ...
      providerIDs: cloneValue(record.providerIDs, [])
    }
  }
